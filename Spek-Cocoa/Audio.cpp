...
//  Spek-Cocoa
//
//  Created by SwanCurve on 02/19/17.
...
{
public:
    AudioFileImpl(AudioError error,
...
AudioFile* Audio::open(const std::string& file_name, int stream)
{
...
//    return std::unique_ptr<AudioFile>(new AudioFileImpl(error,
//                                                        format_context,
//                                                        audio_stream,
//                                                        codec_name,
//                                                        bit_rate,
...
//                                                        ));
    ...
    return (new AudioFileImpl(error,
                              format_context,
                              audio_stream,
                              codec_name,
                              bit_rate,
...
AudioFileImpl::AudioFileImpl(AudioError error,
                             AVFormatContext *format_context,
                             int audio_stream,
                             const std::string& codec_name,
                             int bit_rate,
            ...
            for (int sample = 0; sample < samples; ++sample) {
                ...
                switch (format) {
                    ...
                    case AV_SAMPLE_FMT_S16P:
                        value = reinterpret_cast<int16_t*>(data)[offset] / static_cast<float>(INT16_MAX);
                        break;
                    case AV_SAMPLE_FMT_S32:
                    case AV_SAMPLE_FMT_S32P:
                        value = reinterpret_cast<int32_t*>(data)[offset] / static_cast<float>(INT32_MAX);
                        break;
